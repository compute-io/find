...
* @param {String} [opts.returns] - if `values`, values are returned; if `indices`, indices are returned (default: `indices`)
...
* @returns {Array} array of indices or element values
*/
function find( arr, opts, clbk ) {
	...
	if ( opts.hasOwnProperty( 'k' ) ) {
		...
		if ( typeof k !== 'number' || k !== k ) {
			throw new TypeError( 'find()::invalid input argument. `k` must be numeric.' );
		}
	} else {
		k = len;
	}
	if ( opts.hasOwnProperty( 'returns' ) ) {
		...
		if ( typeof ret !== 'string' || returns.indexOf( ret ) === -1 ) {
			throw new TypeError( 'find()::invalid input argument. `returns` option must be either `values` or `indices`.' );
		}
		if ( ret === 'values' ) {
			flg = true;
		}
